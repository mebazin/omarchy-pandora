...
Item {
  ...
  readonly property string socketPath: runtimeDir + "/mark.pandora/engine.sock"
  ...
  function ensureEngine() {
    if (spawns >= maxSpawns) {
      transportError = "Pandora engine failed to start. See ~/.local/state/mark.pandora/engine.log"
      return
    }
    spawns++
    ticksSinceSpawn = 0
